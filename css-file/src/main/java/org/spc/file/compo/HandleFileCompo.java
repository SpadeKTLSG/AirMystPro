...
import org.spc.file.artifact.FileArtifact;
...
public class HandleFileCompo extends BaseCompo {
     ...
     * @return 返回TR序列
     */
    public String pathTR(node target) {
     ...
     * @return 返回是否找到
     */
    public boolean searchTRbyNode(node root, node target, StringBuilder sb) {
        if (root == null) {
            return false;
        }
     ...
     * @return 返回定位到的节点
     */
    node searchNodebyOrder(node root, String[] dir, int index, StringBuilder sb) {
        ...
        if (fileArtifact.getName(root.fcb).equals(dir[index])) { //比对当前节点的fcb的pathName切分后提取出的文件名和目录数组中的目录是否相同
     ...
     * @return 返回父节点
     */
    public node searchUpperNode(FCB fcb) {

        //1. 拿到FCB按照"目录 : 名称"切分为两块: 目录和名称
        String[] dir = fileArtifact.getPathArray(fcb);
//        String name = getName(fcb);
        ...
        if (dir_temp == null) {
            ...
            throw new FailedException("Ooops! 系统中找不到对应的目录");
        }
     ...
     * @return 返回对应的node
     */
    public node selectTR2Node(FCB fcb) {
        //? 拿到FCB后, 通过String切分判断其位置(树上意义无意义),因此暂时直接调用pathTR返回类似order的路径

        //1. 拿到FCB按照"目录 : 名称"切分为两块: 目录和名称
        String[] dir = fileArtifact.getPathArray(fcb);
        String name = fileArtifact.getName(fcb);
     ...
     * @return 路径管理器中的键
     */
    public Integer findKeyiPM(String pathName) {
        try {
     ...
     * @return 扩展名
     */
    public String selectEM(Integer key) {
     ...
     * @return 返回扩展名管理器中的键
     */
    public Integer bindEM(String extendName) {
     ...
     * @return 扩展名管理器中的键
     */
    public Integer findKeyiEM(String extendName) {
        try {
            List<Integer> keys = fileSyS.extendManager.entrySet().stream()
