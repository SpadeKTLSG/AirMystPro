...
//        setDefaultPM();
        ...
        List<Integer> orderList = handleDiskCompo.getFATOrder();
        ...
        for (Integer single : orderList) {
            ...
            //提取出fcb和Contents
            FCB tempFCB = handleFileCompo.getFileArtifact().bytes2Fcb_AppendPM(fcbBytes); //这里有当前无法实现的重大BUG: 文件系统始终是JAVA对象,重启后无法复原. 只能进行转移
     ...
     * @param root 根节点
     */
    public void setDefaultTR(node root) {
     ...
     * @param fcb 新增对象的FCB
     */
    public void addTR(FCB fcb) { //fcb或者是node皆可
        //? 拿到FCB后, 通过String切分判断其上级已存在node的位置, 封装到searchUpperNode方法中
        node dir_temp = handleFileCompo.searchUpperNode(fcb);
    ...
    public void deleteTR(FCB fcb) {
        ...
        if (Objects.equals(parentNode.fcb.getPathName(), "/:")) { //安全判断
            ...
            throw new FailedException("小子, 你在玩火!");
        }
    ...
    //修改TR节点
    public void alterTR(FCB fcb1, FCB fcb2) {
        ...
        Objects.requireNonNull(handleFileCompo.selectTR2Node(fcb1)).fcb = fcb2;//直接替换内容
     ...
     * @deprecated
     */
    public String selectTR2Path(FCB fcb) {
        return handleFileCompo.pathTR(handleFileCompo.selectTR2Node(fcb));
    }
     ...
     */
    public Object selectTR2Content(FCB fcb) {
        FCB originalFcb = Objects.requireNonNull(handleFileCompo.selectTR2Node(fcb)).fcb;
        String allBlock = txtUtil.read1BlockiTXT(originalFcb.getStartBlock());
     ...
     * 获取初始PM
     */
    public Map<Integer, String> initialPM() {
    ...
    public void setDefaultPM() {
        ...
        for (ROOT_PATH root_path : ROOT_PATH.values())
            bindPM(handleFileCompo.getFileArtifact().str2Path(String.valueOf(root_path)));
    }
     ...
     * @return 路径管理器中的键
     */
    public Integer deletePM(String pathName) {
     ...
     * @return 路径管理器中的键
     */
    public Integer alterPM(String pathName_or, String pathName_new) {
     ...
     * @return FCB的PathName
     */
    public String selectPM(Integer key) {
        return pathManager.get(key);
    }
     ...
     */
    public Map<Integer, String> initialEM() {
     ...
     * <p>按照文档和文件的扩展名进行设置</p>
     */
    public void setDefaultEM() {
        extendManager = initialEM();
        try {//系统自带的直接指定提高效率
            int i = 0;
            ...
            for (String s : AddedEXTEND) {
                handleFileCompo.bindEM(s); //同步用户添加的扩展名
            }
        } catch (Exception e) {
     ...
     * @param A 文件/文件夹对象
     */
    public void addContentFS(Object A) {
     ...
     * @param A 文件/文件夹对象
     */
    public void deleteContentFS(Object A) {
...
//        deleteTR(A);
     ...
     * @param B 修改后的对象
     */
    public void alterContentFS(Object A, Object B) {
     ...
     */
    public Object selectContentFS(Object A) {
        if (A instanceof file file_temp) {
